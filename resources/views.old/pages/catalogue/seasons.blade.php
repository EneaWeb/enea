    ...
    <div class="content-frame">
        ...
        <div class="content-frame-left">
            ...
            @if (\App\Option::active_season() != NULL)
                <div class="widget widget-warning widget-no-subtitle">
                    <div class="widget-subtitle">{!! trans('x.Current Season') !!}</div>
            ...
            @endif
                ...
                <div class="widget widget-success widget-no-subtitle">
                    <span class="widget-subtitle">{!!trans('x.Change active Season')!!}</span>
                    <div class="widget-title">
                        {!!Form::open(['url'=>'/catalogue/season/select', 'method'=>'POST', 'class'=>'form-material'])!!}
                            {!!Form::select('season_id', \App\Season::lists('name', 'id'), '', ['class'=>'form-control', 'style'=>'text-indent:10px; background-color:white', 'onchange'=>'this.form.submit()', 'placeholder'=>trans('x.Please select')])!!}
                        {!!Form::close()!!}
                    </div>                                                     
                </div>
            </div>
            ...
            <div class="panel panel-default">
                <h3 class="list-group-item">{!! trans('x.Show Season')!!}</h3>
                    ...
                    {!! Form::open(['url'=> '/catalogue/seasons/change', 'method'=>'POST', 'class'=>'form-material'])!!}
                        ...
                        <div class="form-group">
                            <select name="season_id" class="form-control" onchange="this.form.submit()">
                                    <option value="">{!!trans('x.Select')!!}</option>
                                @foreach (\App\Season::all() as $s)
                                    <option value="{!!$s->id!!}">{!!$s->name!!}</option>
                                @endforeach
                            </select>
                            <label for="season_id">{!!trans('x.Select Season')!!}</label>
                        </div>          
        ...
        <div class="content-frame-body content-frame-body-right" style="padding-top:0">
            ...
            <div class="content-frame-top">
                ...
                <h1>{!!trans('x.Season')!!} <span class="btn btn-success btn-lg">{!!$season->name!!}</span></h1>
