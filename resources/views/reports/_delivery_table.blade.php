            ...
            <tbody>
                @foreach($variation_ids as $variation_id)
                    ...
                    <tr>
                        ...
                        <td>
                            <strong>{!!\App\ProductVariation::find($variation_id)->product->prodmodel->name!!}<br>{!!\App\ProductVariation::find($variation_id)->product->name!!}<br><span style="color:{!!(\App\ProductVariation::find($variation_id)->color->hex == '#ffffff') ? '#ffffff; background-color:#D9D9D9; padding:2px' : \App\ProductVariation::find($variation_id)->color->hex !!};"></span></strong>
                        </td>
                        @foreach (\App\Size::all() as $size)
